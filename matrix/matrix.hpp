...
class Matrix {
    ...
    static std::string to_string(const Shape& s) {
    ...
    void checkShapeMatch(const Matrix<T>& rhs) const {
        if (!(shape_ == rhs.shape_)) {
            throw std::runtime_error("Shape mismatch: lhs is " + to_string(shape_) + ", rhs is " + to_string(rhs.shape_));
        }
    }
    ...
    */
    ...
    /*
    applys dot product matrix matrix multiplication
